...
public class BigTableReader extends SSTableReader
{
    ...
    protected RowIndexEntry getPosition(PartitionPosition key, Operator op, boolean updateCacheAndStats, boolean permitMatchPastLast)
    {
        if (op == Operator.EQ)
        {
            ...
            {
                logger.info("Bloom filter allows skipping sstable {}", descriptor.generation);
                return null;
            }
        }
        ...
        try (FileDataInput in = ifile.createReader(sampledPosition))
        {
            ...
            while (!in.isEOF())
            {
                ...
                else
                {
                    ...
                    if (v < 0)
                    {
                        logger.info("Partition index lookup allows skipping sstable {}", descriptor.generation);
                        return null;
                    }
                }
        ...
        if (op == SSTableReader.Operator.EQ && updateCacheAndStats)
            bloomFilterTracker.addFalsePositive();
        logger.info("Partition index lookup complete (bloom filter false positive) for sstable {}", descriptor.generation);
        return null;
    }
